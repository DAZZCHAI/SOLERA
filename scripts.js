...
document.querySelectorAll(".mm-link, .mobile-menu .btn, .mobile-menu__close").forEach(a=> a.addEventListener("click", ()=> toggleMenu(false)));
...
/* ---------- Testimonios: dots del carrusel ---------- */
(function(){
  const grid = document.querySelector('.testi__grid');
  const dotsWrap = document.getElementById('testiDots');
  if(!grid || !dotsWrap) return;
  const cards = grid.querySelectorAll('.testi__card');
  if(cards.length === 0) return;
  // Crear dots
  cards.forEach((_, i) => {
    const d = document.createElement('button');
    d.className = 'testi__dot' + (i === 0 ? ' active' : '');
  ...
  grid.addEventListener('scroll', () => {
    ...
    dotsWrap.querySelectorAll('.testi__dot').forEach((d, i) => d.classList.toggle('active', i === closest));
  }, {passive:true});
})();
